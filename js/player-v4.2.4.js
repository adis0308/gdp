...
var $timez = $("#timez"),
    ...
    jwp = {
        ...
        loadSuccessCallback: function (res) {
            if (pConf.blockADB && gdpAdsError !== undefined) {
                jwp.destroy();
                adblockerMessage();
                return;
            }
    ...
    gdPlyr = {
        ...
        loadSuccessCallback: function (res) {
            ...
            retryNumber = getRetryNumber() + 1;
            if (pConf.blockADB && gdpAdsError !== undefined) {
                var gdPlyrIntv = setInterval(function () {
                    gdPlyr.destroy();
                    if (gdPlyr.player === null) {
                        adblockerMessage();
                        clearInterval(gdPlyrIntv);
                    }
                }, 100);
                return;
            }
...
function visitDirectAds() {
    if (!pConf.disableDirectAds && pConf.visitAdsOnplay && pConf.directAdsLink !== "" && pConf.directAdsLink !== "#") {
        ...
        if (pConf.showIframeAds) {
            var adsTm = setTimeout(function () {
